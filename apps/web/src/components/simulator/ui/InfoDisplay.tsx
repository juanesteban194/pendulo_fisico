...
import { computeDerived } from '@pendulo/physics'
import type { PendulumState, PendulumParams, DerivedQuantities } from '@pendulo/physics'
...
function Row({ label, value, unit, color, mono = true }: {
  ...
  return (
    <div style={s.row}>
      ...
      <span style={{ ...s.rowValue, color: color ?? T.text, fontFamily: mono ? 'ui-monospace, monospace' : 'inherit' }}>
        {value}{unit && <span style={s.rowUnit}> {unit}</span>}
      </span>
    </div>
  )
}
...
export function InfoDisplay() {
  ...
  const Q_val  = d.b > 1e-12 && d.f > 0 ? (2 * Math.PI * d.I * d.f) / d.b : Infinity
  ...
  return (
    <div style={s.panel}>
      ...
      <Row label="T — período"    value={isFinite(d.T) ? d.T.toFixed(4) : '∞'} unit={isFinite(d.T) ? 's' : ''} color={T.orange} />
      <Row label="f — frecuencia" value={d.f.toFixed(4)}       unit="Hz" />
      <Row label="I — inercia"    value={d.I.toExponential(4)} unit="kg·m²" />
      <Row label="d — CM → pivote" value={d.d.toFixed(4)} unit="m" color={d.stability === 'unstable' ? '#dc2626' : T.text} />
      <Row label="Leq — equiv."   value={isFinite(d.Leq) ? d.Leq.toFixed(4) : '—'} unit={isFinite(d.Leq) ? 'm' : ''} color={T.dim} />
      <div style={s.row}>
...
const s: Record<string, CSSProperties> = {
  panel: { display: 'flex', flexDirection: 'column', gap: '6px', padding: '10px 12px', height: '100%', overflowY: 'auto', boxSizing: 'border-box' },
  ...
  badge: { fontSize: '10px', fontWeight: 700, padding: '2px 8px', borderRadius: '5px', letterSpacing: '0.03em' },
}
