...
const T = {
  ...
  amber:   '#d97706', green:   '#16a34a', indigo:  '#4f46e5',
  red:     '#dc2626', teal:    '#0d9488',
  text:    '#1e293b', muted:   '#64748b', dim:     '#94a3b8',
  border:  'rgba(15,23,42,0.09)', bg: '#f8fafc',
}

interface SliderProps {
  label: string; symbol: string; unit: string
  value: number; min: number; max: number; step: number
  digits?: number; color?: string; onChange: (v: number) => void
}
  ...
  return (
    <div style={s.sliderWrap}>
      <div style={s.sliderHeader}>
        <span style={s.paramLabel}>{label}<span style={s.paramSymbol}> {symbol}</span></span>
        <span style={{ ...s.paramValue, color }}>
          {value.toFixed(digits)}<span style={s.paramUnit}> {unit}</span>
        </span>
      </div>
    ...
    <div style={s.panel}>
      <div style={s.header}>
        ...
        <div style={s.headerBtns}>
          <button
            style={{ ...s.iconBtn, background: running ? '#fef3c7' : '#dcfce7', color: running ? T.amber : T.green, borderColor: running ? '#fde68a' : '#86efac' }}
            onClick={() => setRunning(!running)} title={running ? 'Pausar' : 'Reanudar'}
          >{running ? '⏸' : '▶'}</button>
          <button style={{ ...s.iconBtn, background: '#ede9fe', color: T.indigo, borderColor: '#c4b5fd' }}
            onClick={reset} title="Reiniciar">↺</button>
          <button style={{ ...s.iconBtn, ...s.labBtn }} onClick={() => setParams(LAB_PARAMS)}
            title="Restaurar parámetros del lab">Lab</button>
        </div>
      </div>
      ...
      <Section title="Geometría del péndulo">
        <Slider label="Longitud"     symbol="L"  unit="m"  value={params.L}           min={0.05} max={3.0}  step={0.01}  digits={2} onChange={v => setParams({ L: v })} />
        <Slider label="Masa extremo" symbol="mᵣ" unit="kg" value={params.mr}          min={0.01} max={10.0} step={0.05}  digits={2} onChange={v => setParams({ mr: v })} />
        <Slider label="Masa barra"   symbol="m"  unit="kg" value={params.m}           min={0.001} max={5.0} step={0.025} digits={3} color={T.blue} onChange={v => setParams({ m: v })} />
        <Slider label="Pos. pivote"  symbol="a"  unit="m"  value={params.pivotOffset} min={0}    max={params.L} step={0.005} digits={3} color={T.amber} onChange={v => setParams({ pivotOffset: v })} />
      </Section>

      <Section title="Condición inicial">
        <Slider label="Ángulo inicial" symbol="θ₀" unit="°" value={theta0Deg} min={1} max={90} step={1} digits={0} color={T.violet} onChange={v => setParams({ theta0: v * (Math.PI / 180) })} />
      </Section>
      ...
      <Section title="Medio y temperatura">
        <div style={s.fluidGrid}>
          {fluids.map(f => {
            ...
            return (
              <button key={f.id} style={{ ...s.fluidBtn, ...(active ? s.fluidBtnActive : {}) }}
        ...
        <Slider label="Temperatura" symbol="T" unit="°C" value={params.tempC} min={0} max={100} step={1} digits={0} color={T.red} onChange={v => setParams({ tempC: v })} />
      </Section>

      <Section title="Entorno gravitacional">
        <Slider label="Gravedad" symbol="g" unit="m/s²" value={params.g} min={1.6} max={24.8} step={0.01} digits={2} color={T.teal} onChange={v => setParams({ g: v })} />
...
const s: Record<string, CSSProperties> = {
  panel: { display: 'flex', flexDirection: 'column', gap: '2px', padding: '10px 12px', height: '100%', overflowY: 'auto', boxSizing: 'border-box' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' },
  panelTitle: { fontSize: '11px', fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.1em' },
  ...
  fluidGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '5px' },
  fluidBtn: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', padding: '7px 4px', fontSize: '10px', fontWeight: 500, color: T.muted, background: T.bg, border: `1px solid ${T.border}`, borderRadius: '7px', cursor: 'pointer', lineHeight: 1.2 },
  fluidBtnActive: { background: '#fff7ed', borderColor: '#fdba74', color: T.orange },
  fluidIcon: { fontSize: '14px', lineHeight: '1' },
  presetGrid: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '4px' },
  presetBtn: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', padding: '6px 2px', background: T.bg, border: `1px solid ${T.border}`, borderRadius: '7px', cursor: 'pointer' },
  presetBtnActive: { background: '#f0fdf9', borderColor: '#5eead4' },
}
