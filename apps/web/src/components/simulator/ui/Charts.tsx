...
const TOOLTIP_STYLE: CSSProperties = {
  background: 'rgba(255,255,255,0.97)', border: '1px solid rgba(15,23,42,0.1)',
  borderRadius: '6px', fontSize: '11px', color: '#0f172a', padding: '4px 8px',
}
      ...
      <ChartCard title="θ(t) — posición angular" subtitle={`últimos ${WINDOW_SECS} s`}>
        <ResponsiveContainer width="100%" height={96}>
          <LineChart data={thetaFrames} margin={MARGIN}>
            ...
            <XAxis dataKey="time" tick={TICK} label={{ value: 't (s)', position: 'insideBottomRight', offset: -2, style: TICK }} />
            <YAxis tick={TICK} label={{ value: '°', angle: -90, position: 'insideLeft', style: TICK }} />
            ...
            {showAdvanced && (
              <>
                <Line type="monotone" dataKey="envUpper" name="env+" stroke={GRAY} strokeWidth={1} strokeDasharray="3 3" dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="envLower" name="env−" stroke={GRAY} strokeWidth={1} strokeDasharray="3 3" dot={false} isAnimationActive={false} />
              </>
            )}
            <Line type="monotone" dataKey="theta" stroke={VIOLET} strokeWidth={1.5} dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      <ChartCard title="ω(t) — velocidad angular" subtitle={`últimos ${WINDOW_SECS} s`}>
        <ResponsiveContainer width="100%" height={96}>
          <LineChart data={windowedFrames} margin={MARGIN}>
            ...
            <YAxis tick={TICK} label={{ value: 'rad/s', angle: -90, position: 'insideLeft', style: { ...TICK, fontSize: 8 } }} />
            <ReferenceLine y={0} stroke={EQUI} strokeDasharray="4 3" />
            <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number) => [`${v.toFixed(4)} rad/s`, 'ω']} labelFormatter={(t: number) => `t = ${t} s`} />
            <Line type="monotone" dataKey="omega" stroke={BLUE} strokeWidth={1.5} dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      <ChartCard title="Fase — θ vs ω">
        <ResponsiveContainer width="100%" height={96}>
          <ScatterChart margin={MARGIN}>
            <CartesianGrid strokeDasharray="2 3" stroke={GRID} />
            <XAxis dataKey="theta" type="number" name="θ" tick={TICK} label={{ value: 'θ (°)', position: 'insideBottomRight', offset: -2, style: TICK }} />
            <YAxis dataKey="omega" type="number" name="ω" tick={TICK} label={{ value: 'ω', angle: -90, position: 'insideLeft', style: TICK }} />
            <ReferenceLine x={0} stroke={EQUI} strokeDasharray="4 3" />
            <ReferenceLine y={0} stroke={EQUI} strokeDasharray="4 3" />
            <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number, name: string) => name === 'θ' ? [`${v.toFixed(2)}°`, 'θ'] : [`${v.toFixed(3)} rad/s`, 'ω']} />
            <Scatter data={phaseData} line={{ stroke: GREEN, strokeWidth: 1.2 }} lineType="joint" shape={() => null as unknown as React.ReactElement} fill={GREEN} isAnimationActive={false} />
          </ScatterChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      <ChartCard title="Energía mecánica">
        <ResponsiveContainer width="100%" height={96}>
          <LineChart data={frames} margin={MARGIN}>
            <CartesianGrid strokeDasharray="2 3" stroke={GRID} />
            <XAxis dataKey="time" tick={TICK} />
            <YAxis tick={TICK} label={{ value: 'J', angle: -90, position: 'insideLeft', style: TICK }} />
            <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number, name: string) => [`${v.toExponential(3)} J`, name]} labelFormatter={(t: number) => `t = ${t} s`} />
            <Legend iconSize={8} wrapperStyle={{ fontSize: '9px', color: MUTED }} />
            <Line type="monotone" dataKey="Ec"     name="Ec cinética"  stroke={ORANGE} strokeWidth={1.5} dot={false} isAnimationActive={false} />
            <Line type="monotone" dataKey="Ep"     name="Ep potencial" stroke={BLUE}   strokeWidth={1.5} dot={false} isAnimationActive={false} />
            <Line type="monotone" dataKey="Etotal" name="E total"      stroke={GREEN}  strokeWidth={1}   strokeDasharray="4 2" dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
    </div>
...
const s: Record<string, CSSProperties> = {
  grid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: '8px', padding: '8px', height: '100%', boxSizing: 'border-box', overflow: 'hidden' },
  card: { display: 'flex', flexDirection: 'column', background: '#ffffff', borderRadius: '8px', padding: '8px 10px 4px', border: '1px solid rgba(15,23,42,0.07)', overflow: 'hidden', boxShadow: '0 1px 3px rgba(15,23,42,0.05)' },
  cardHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2px' },
  cardTitle: { margin: 0, fontSize: '11px', fontWeight: 700, color: 'rgba(51,65,85,0.85)', letterSpacing: '0.01em' },
  cardSub: { fontSize: '9px', color: 'rgba(71,85,105,0.45)', fontStyle: 'italic', flexShrink: 0 },
