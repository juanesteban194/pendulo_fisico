...
export interface PendulumParams {
  L:           number   // longitud TOTAL de la barra (m)        — exp: 0.25
  ...
  pivotOffset: number   // distancia del extremo superior al pivote (m), [0, L]
                        //   0 = pivote en el extremo superior (configuración estándar)
                        //   L = pivote en el extremo inferior (junto a la masa)
}
...
export interface DerivedQuantities {
  ...
  d:         number   // distancia pivote→CM con signo (m): +abajo, −arriba (inestable)
  M:         number   // masa total (kg)
  T:         number   // período teórico (s) — Infinity si d ≤ 0 (no oscila)
  f:         number   // frecuencia (Hz) — 0 si T = ∞
  Leq:       number   // longitud equivalente (m) — Infinity si d ≤ 0
  Ec:        number   // energía cinética (J)
  Ep:        number   // energía potencial (J) — puede ser negativa si d < 0
  ...
  stability: 'stable' | 'critical' | 'unstable'  // según el signo de d
}
